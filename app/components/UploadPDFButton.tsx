...
//       // You might want to add your own logic here, such as sending the file to a server
//     } else {
...
//       try {
//         const response = await fetch("https://consciouscart-backend-production.up.railway.app/api/upload-file", {
//           method: "POST",
//           body: formData,
//         });
  
//         if (response.ok) {
//           setUploadMessage(`Success: The PDF "${file.name}" has been uploaded successfully!`);
...
//           setUploadMessage(`Error: Failed to upload PDF. ${errorData.message}`);
//         }
//       } catch (error) {
    ...
    if (file && file.type === "application/pdf") {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setUploadMessage(`Failed to upload PDF: ${errorData.message}`);
  ...
  return (
    <>
      ...
      {uploadMessage && (
        <p
          style={{
            color: isSuccess === true ? "green" : isSuccess === false ? "red" : "black",
          }}
        >
          {uploadMessage}
        </p>
